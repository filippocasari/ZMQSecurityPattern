...
int main (void)
{
    FILE *password = fopen ( "./passwords", "w");
    fprintf (password, "filippo=ciao\n");
    fclose(password);
    //  Create and start authentication engine
    zactor_t *auth = zactor_new (zauth,NULL);
    zstr_send(auth,"VERBOSE");
    zsock_wait(auth); //zauth is synchronous , hence any command/function must be followed by a wait function
    ...
    zsock_set_plain_username (client, "filippo");
    zsock_set_plain_password (client, "ciao");
